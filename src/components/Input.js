...
export default function Input() {
  /* STEP 1 
  Create a slice of state called 'inputValue' and its 
  'setInputValue' buddy.
  We should initialize this state to the empty string.
  */
  const [inputValue, setInputValue] = useState ('')
  const changeInput = evt => {
    // When the input changes, its whole value can be found inside 
    //the event object.
  ...
  const reset = () => {
    /* STEP 5 
    Set the input value in state to be empty string.
     The reset still won't work, though! See the next step.
    */
    setInputValue('')
  };
